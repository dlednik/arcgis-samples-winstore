        ...
        private void LocationProvider_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            if (providerSelector != null)
            {
                if (providerSelector.SelectedIndex == 0)
                    MyMap.LocationDisplaySettings.LocationProvider = new ESRI.ArcGIS.Runtime.Xaml.SystemLocationProvider();
                else if (providerSelector.SelectedIndex == 1)
                    MyMap.LocationDisplaySettings.LocationProvider = new RandomProvider();
            }
        }
    }
    public class RandomProvider : ILocationProvider
    {
        ...
        private void timer_Tick(object sender, object e)
        {
            if (oldPosition == null)
            {
                ...
                {
                    Location = new ESRI.ArcGIS.Runtime.MapPoint(StartLongitude, StartLatitude) { SpatialReference = new ESRI.ArcGIS.Runtime.SpatialReference(4326) },
                    Speed = 0,
                    Course = 0,
